...
* **RFA01 (Router e Direcionamento):** O Agente deve processar o texto, identificar as intenções primariamente com LLMs e encaminhar as diretivas a ferramentas locais utilizando a interface de *Function/Tool Calling*. Quando a pergunta depender de periodo e as datas nao forem informadas, o agente deve pedir clarificacao antes de chamar qualquer tool. Prompting simples onde toda base de dados é jogada por texto não será tolerado.
* **RFA02 (Análise de Volume):** A rede deverá acionar uma busca no GCP reportando a contagem de usuarios por `traffic_source` no periodo informado.
